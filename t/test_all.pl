...
my $testlog      = '/tmp/nagios-mod-gearman_test.log';
...
for my $test (@tests) {
    ...
    $cmd = "yes | valgrind --tool=memcheck --leak-check=yes --leak-check=full --show-reachable=yes --track-origins=yes --gen-suppressions=yes --error-limit=no --log-file=$vallog ./$test >$testlog 2>&1";
    ok(1, $cmd);
    `$cmd`;
