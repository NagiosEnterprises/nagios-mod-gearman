...
# check requirements
ok(-f './nagios-mod-gearman-worker', 'worker present') or BAIL_OUT("no worker!");
...
for(1..10) {
    $gearman_top_out = `./nagios-gearman-top -b -H localhost:$TESTPORT 2>&1`;
...
# fill the queue
open(my $ph, "|./nagios-send-gearman --server=localhost:$TESTPORT --result_queue=eventhandler") or die("failed to open nagios-send-gearman: $!");
my $t0 = [gettimeofday];
...
`>worker.log`;
$t0 = [gettimeofday];
my $cmd = "./nagios-mod-gearman-worker --server=localhost:$TESTPORT --debug=0 --max-worker=1 --encryption=off --daemon --pidfile=./worker.pid --logfile=./worker.log";
system($cmd);
...
sub wait_for_empty_queue {
    my $queue = shift;
    open(my $ph, "./nagios-gearman-top -b -i 0.05 -H localhost:$TESTPORT |") or die("cannot launch nagios-gearman-top: $!");
    while(my $line = <$ph>) {
