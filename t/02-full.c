...
void *start_worker(void*data) {
    ...
    if(pid == 0) {
        ...
        if(key != NULL) {
            ...
            execl("./nagios-mod-gearman-worker", "./nagios-mod-gearman-worker", "debug=2", encryption,      logf, "max-worker=1", options, (char *)NULL);
        } else {
            execl("./nagios-mod-gearman-worker", "./nagios-mod-gearman-worker", "debug=2", "encryption=no", logf, "max-worker=1", options, (char *)NULL);
        }
        perror("nagios-mod-gearman-worker");
        exit(1);
    }
    else {
        worker_pid = pid;
...
void check_no_worker_running(char* logfile) {
    ...
    snprintf(cmd, 150, "ps -efl 2>/dev/null | grep -v grep | grep '%s' | grep nagios-mod-gearman-worker", username);
...
int main (__attribute__((unused)) int argc, __attribute__((unused)) char **argv, __attribute__((unused)) char **env) {
     ...
     * nagios-send-gearman
     */
    snprintf(cmd, 150, "./nagios-send-gearman --server=127.0.0.1:%d --key=testtest --host=test --service=test --message=test --returncode=0", GEARMAND_TEST_PORT);
    rrc = real_exit_code(run_check(cmd, &result, &error));
    cmp_ok(rrc, "==", 0, "cmd '%s' returned rc %d", cmd, rrc);
    like(result, "^\\s*$", "output from ./nagios-send-gearman");
    gm_free(result);
    gm_free(error);
     ...
     * nagios-send-multi
     */
    snprintf(cmd, 150, "./nagios-send-multi --server=127.0.0.1:%d --host=blah < t/data/send_multi.txt", GEARMAND_TEST_PORT);
    rrc = real_exit_code(run_check(cmd, &result, &error));
    cmp_ok(rrc, "==", 0, "cmd '%s' returned rc %d", cmd, rrc);
    like(result, "nagios-send-multi OK: 2 check_multi child checks submitted", "output from ./nagios-send-multi");
    gm_free(result);
    gm_free(error);
     ...
     */
    snprintf(cmd, 150, "./nagios-check-gearman -H 127.0.0.1:%d -s check -a -q worker_test", GEARMAND_TEST_PORT);
    rrc = real_exit_code(run_check(cmd, &result, &error));
    cmp_ok(rrc, "==", 0, "cmd '%s' returned rc %d", cmd, rrc);
    like(result, "nagios-check-gearman OK - sending background job succeded", "output from ./nagios-check-gearman");
