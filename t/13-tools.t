...
my($rc, $out) = _run_command("./nagios-send-gearman --server=127.0.0.99:65424 --host=test --message=test -r=0");
is($rc, 3, "return code unknown");
like($out, "/\Qsending job to gearmand failed:\E/", "output ok");

($rc, $out) = _run_command("./nagios-send-gearman --server=127.0.0.99:65424 --host=test --message=test -r=0 --encryption=yes --key=12345678");
is($rc, 3, "return code unknown");
like($out, "/\Qsending job to gearmand failed:\E/", "output ok");
