...
int nebmodule_init( int flags, char *args, nebmodule *handle ) {
    ...
    neb_set_module_info( gearman_module_handle, NEBMODULE_MODINFO_TITLE,   "Nagios-Mod-Gearman" );
    neb_set_module_info( gearman_module_handle, NEBMODULE_MODINFO_AUTHOR,  "Nagios Development Team" );
    neb_set_module_info( gearman_module_handle, NEBMODULE_MODINFO_TITLE,   "Copyright (c) 2024 Nagios Development Team" );
    ...
    if(!(event_broker_options & BROKER_PROGRAM_STATE)) {
        gm_log( GM_LOG_ERROR, "nagios-mod-gearman needs BROKER_PROGRAM_STATE (%i) event_broker_options enabled to work\n", BROKER_PROGRAM_STATE );
        broker_option_errors++;
    }
    if(!(event_broker_options & BROKER_TIMED_EVENTS)) {
        gm_log( GM_LOG_ERROR, "nagios-mod-gearman needs BROKER_TIMED_EVENTS (%i) event_broker_options enabled to work\n", BROKER_TIMED_EVENTS );
        broker_option_errors++;
    }
    if(    (    mod_gm_opt->perfdata != GM_DISABLED
        ...
        gm_log( GM_LOG_ERROR, "nagios-mod-gearman needs BROKER_HOST_CHECKS (%i) event_broker_options enabled to work\n", BROKER_HOST_CHECKS );
        broker_option_errors++;
    }
    if(    (    mod_gm_opt->perfdata != GM_DISABLED
        ...
        gm_log( GM_LOG_ERROR, "nagios-mod-gearman needs BROKER_SERVICE_CHECKS (%i) event_broker_options enabled to work\n", BROKER_SERVICE_CHECKS );
        broker_option_errors++;
    }
    if(mod_gm_opt->events == GM_ENABLED && !(event_broker_options & BROKER_EVENT_HANDLERS)) {
        gm_log( GM_LOG_ERROR, "nagios-mod-gearman needs BROKER_EVENT_HANDLERS (%i) event_broker option enabled to work\n", BROKER_EVENT_HANDLERS );
        broker_option_errors++;
    }
    ...
    if(strlen(GIT_HASH) > 0)
        logit(NSLOG_INFO_MESSAGE, FALSE, "nagios-mod-gearman: initialized version %s (build: %s) (libgearman %s)\n", GM_VERSION, GIT_HASH, gearman_version());
    else
        logit(NSLOG_INFO_MESSAGE, FALSE, "nagios-mod-gearman: initialized version %s (libgearman %s)\n", GM_VERSION, gearman_version());
...
int nebmodule_deinit( int flags, int reason ) {
    ...
    log_core( NSLOG_INFO_MESSAGE, "nagios-mod-gearman: deinitializing\n" );
...
/* insert results list into nagios core */
static void move_results_to_core(void) {
...
static int handle_process_events( int event_type, void *data ) {
    ...
    /* verify names of supplied groups
        * this cannot be done before nagios has finished reading his config
...
static int handle_host_check( int event_type, void *data ) {
    ...
    if(mod_gm_opt->orphan_host_checks == GM_ENABLED && check_options & CHECK_OPTION_ORPHAN_CHECK) {
        ...
        if ( ( chk_result = ( check_result * )gm_malloc( sizeof *chk_result ) ) == 0 )
            return NEBERROR_CALLBACKCANCEL;
        snprintf( temp_buffer,GM_MAX_OUTPUT-1,"(host check orphaned, is the nagios-mod-gearman worker on queue '%s' running?)\n", target_queue);
...
static int handle_svc_check( int event_type, void *data ) {
    ...
    if(mod_gm_opt->orphan_service_checks == GM_ENABLED && check_options & CHECK_OPTION_ORPHAN_CHECK) {
        ...
        if ( ( chk_result = ( check_result * )gm_malloc( sizeof *chk_result ) ) == 0 )
            return NEBERROR_CALLBACKCANCEL;
        snprintf( temp_buffer,GM_MAX_OUTPUT-1,"(service check orphaned, is the nagios-mod-gearman worker on queue '%s' running?)\n", target_queue);
        init_check_result(chk_result);
