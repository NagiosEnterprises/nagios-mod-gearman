...
/* print version */
void print_version() {
    printf("nagios-check-gearman: version %s running on libgearman %s\n", GM_VERSION, gearman_version());
    printf("\n");
    exit( STATE_UNKNOWN );
}
...
void print_usage() {
    printf("usage:\n");
    printf("\n");
    printf("nagios-check-gearman [ -H=<hostname>[:port]         ]\n");
    printf("                     [ -t=<timeout>                 ]\n");
    ...
    printf("\n");
    printf("%%>./nagios-check-gearman -H localhost -q host\n");
    printf("nagios-check-gearman OK - 0 jobs running and 0 jobs waiting. Version: 0.14\n");
    printf("\n");
    printf("Check worker:\n");
    printf("\n");
    printf("%%> ./nagios-check-gearman -H <job server hostname> -q worker_<worker hostname> -t 10 -s check\n");
    printf("nagios-check-gearman OK - host has 5 worker and is working on 0 jobs\n");
    printf("%%> ./nagios-check-gearman -H <job server hostname> -q perfdata -t 10 -x\n");
    printf("nagios-check-gearman CRITICAL - Queue perfdata has 155 jobs without any worker. |'perfdata_waiting'=155;10;100;0 'perfdata_running'=0 'perfdata_worker'=0;25;50;0\n");
    printf("\n");
    printf("Check result worker:\n");
    printf("%%> ./nagios-check-gearman -H <job server hostname> -q check_results -t 10 -s check\n");
