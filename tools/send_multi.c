...
int main (int argc, char **argv) {
    ...
    if(client == NULL ) {
        printf("nagios-send-multi UNKNOWN: cannot start client\n");
        exit(STATE_UNKNOWN);
    }
    ...
    if ( mod_gm_opt->dupserver_num > 0 ) {
        ...
        if(client_dup == NULL) {
            printf("nagios-send-multi UNKNOWN: cannot start client for duplicate server\n");
            exit(STATE_UNKNOWN);
        }
    }
    ...
    if (rc == 0) {
        printf( "nagios-send-multi UNKNOWN: %d check_multi child checks submitted\n", rc );
        rc=STATE_UNKNOWN;
    }
    else if (rc > 0) {
        printf( "nagios-send-multi OK: %d check_multi child check%s submitted\n", rc, (rc>1)?"s":"" );
        rc=STATE_OK;
    } else {
        rc*=-1;
    }
...
void print_usage() {
    printf("usage:\n");
    printf("\n");
    printf("nagios-send-multi   [ --debug=<lvl>            ]\n");
    printf("                    [ --help|-h                ]\n");
    printf("\n");
...
/* print version */
void print_version() {
    printf("nagios-send-multi: version %s running on libgearman %s\n", GM_VERSION, gearman_version());
    printf("\n");
    exit( STATE_UNKNOWN );
}
    ...
    do {
        ...
        if ((bufstart=(char *)memmem(buffer,buflen,"<CHILD>",strlen("<CHILD>"))) != NULL) {
            ...
            /* start <CHILD> tag found, but no closing tag </CHILD>, buffer too small? */
                ...
                buflen=0L;
                printf("nagios-send-multi UNKNOWN: no closing tag </CHILD> within buffer, buffer size too small? discarding buffer, %ld bytes now\n", buflen);
        ...
        /* neither <CHILD> nor </CHILD> found, discard buffer */
            ...
            if (!count) {
                ...
                if (buffer[i] == '\0' && i) {
                    printf("nagios-send-multi UNKNOWN: error msg in input buffer: %s\n", buffer);
                    return -STATE_UNKNOWN;
                }
            }
...
int read_child_check(char *bufstart, char *bufend, struct timeval * end_time) {
    ...
    /* end time is the execution time of nagios-send-multi itself */
