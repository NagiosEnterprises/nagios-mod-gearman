...
int main (int argc, char **argv) {
    ...
    if(mod_gm_opt->daemon_mode == GM_ENABLED) {
        ...
        /* we are the child process */
            ...
            gm_log( GM_LOG_INFO, "nagios-mod-gearman worker daemon started with pid %d\n", getpid());
        ...
        gm_log( GM_LOG_INFO, "nagios-mod-gearman worker started with pid %d\n", getpid());
    }
...
void print_usage() {
    ...
    printf("\n");
    printf("Nagios-Mod-Gearman worker executes host- and servicechecks.\n");
    printf("\n");
...
void clean_exit(int sig) {
    ...
    gm_log( GM_LOG_INFO, "nagios-mod-gearman worker exited\n");
    mod_gm_free_opt(mod_gm_opt);
    exit( EXIT_SUCCESS );
}
...
/* print version */
void print_version() {
    printf("nagios-mod-gearman-worker: version %s running on libgearman %s\n", GM_VERSION, gearman_version());
    printf("\n");
    exit( STATE_UNKNOWN );
}
